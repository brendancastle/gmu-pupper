...
cone_id = p.loadURDF("cone.urdf",[1.9,-0.1,0.0])
...
while p.isConnected():
    ...
    if dynamic_cone and time.time() > time_start + time_between:
        p.removeBody(cone_id)
        x_rand = random.uniform(-1,3)
        y_rand = random.uniform(-3,1)
        cone_id = p.loadURDF("cone.urdf",[x_rand,y_rand,0.0]) 
        time_start = time.time()
